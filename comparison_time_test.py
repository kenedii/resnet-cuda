...
class CustomFusedConvBnRelu(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, padding=1):
        ...
        # -----------------------------
        # Dual running stats (FIX)
        # -----------------------------
        # Training (cuDNN BN expects double / void*)
        self.register_buffer(
            'running_mean_d',
            torch.zeros(out_channels, dtype=torch.float64, device=device)
        )
        self.register_buffer(
            'running_var_d',
            torch.ones(out_channels, dtype=torch.float64, device=device)
        )

        # Inference (kernel expects float*)
        self.register_buffer(
            'running_mean_f',
        ...
        self.register_buffer(
            'running_var_f',
            torch.ones(out_channels, dtype=torch.float32, device=device)
        )

    def forward(self, x):
        save_mean = torch.zeros(
            self.out_channels,
            dtype=torch.float32,
            device=x.device,
            requires_grad=False
        ...
        # Select correct running stats
        if self.training:
            running_mean = self.running_mean_d
            running_var = self.running_var_d
        else:
            running_mean = self.running_mean_f
            running_var = self.running_var_f
